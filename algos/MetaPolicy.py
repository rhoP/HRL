...
from utils.viz            import (plot_training_curves, plot_skeleton_topology,
                                   save_iteration_visuals, run_demos,
...
def phase2_build_potential(
    ...
    if not raw:
        ...
        raw = {k: {"state": v} for k, v in raw.items()}
    meta_subgoals = {
        k: {"state": np.asarray(v["state"], dtype=np.float32)}
...
def collect_with_meta_policy(
    ...
    for _ in range(num_episodes):
        ...
        while not done and t < max_steps:
            ...
            replay_buffer.push(s_arr, a_np, r,
                                np.asarray(s_next, dtype=np.float32), done, task.id,
                                terminated=terminated)
    ...
    if os.path.isdir(best_dir) and n_demos > 0:
        ...
        try:
            ...
            demo_mp, _, demo_tp, demo_skel = restore_models(
                best_ckpt, state_dim, action_dim,
                discrete=discrete, device=device,
            )
            run_demos(
                demo_mp, demo_tp, demo_skel, task_distribution,
                save_dir=os.path.join(save_dir, "demos"),
                n_demos=n_demos, render=True, gamma=gamma, device=device,
            )
...
_DEFAULT_TASKS = ["reach-v3", "push-v3"]
...
if __name__ == "__main__":
    ...
    if args.demo_only:
        ...
        run_demos(mp, tp, skel, dist,
                  save_dir=os.path.join(args.save_dir, "demos"),
                  n_demos=args.n_demos, render=not args.no_render,
                  device=args.device)
    else:
        main_meta_rl_loop(
